#-->cabeçalho sujeito a mudança
#---------------------------------------------------------
#Código utilizado para implementação do sistema automático
#de rastreamento de campo próximo
#Autor: André Luiz
#
#Versão 1.0
#---------------------------------------------------------
...
class main_window(Frame):
    ...
    rows_disp = 10.35    #Número de linhas apresentado
    cols_disp = 7.75    #Número de colunas apresentado
    ...
    #Função de movimento através dos botões de controle
    ...
    #Função para verificar se o instrumento está medindo     
    def verifica_medicao(self):
        #Caso esteja medindo acusa erro e retorna verdadeiro
        if (self.flag_medindo):
            messagebox.showwarning(title="Erro Ação impossivel",
                                   message="Não é possivel realizar esta função\ndurante a medição")
            return True
        else:
            return False
    
    #Função verifica se string contém somente números maiores que zero     
    ...
    def end_point(self):    #-->Dá pra melhorar juntado star_point com end_point passando para função se é start ou end
        if (self.verifica_medicao()):
            return
        xyz=controle_cnc.current_pos(self.serial_cnc)
    ...
    #Função para atualizar passo entre medidas
    ...
    def medir_ponto(self,row,col):
        ...
        #Verifica se ponto superior esquerdo foi definido e atribui as variáveis
        #de coordenada
        try:
    ...
    #Função comunicação com analisador para definição de
    ...
    def att_freq(self):
        ...
        #Verifica se string contem somente números maiores que zero
    ...
    def medicao(self):
        if (self.verifica_medicao()):
            return
        #Verifica se ponto superior esquerdo foi definido e atribui a variáveis
        #de coordenada
        try:
            if(self.start_point_x<self.end_point_x): x = float(self.start_point_x)
            else: x = float(self.end_point_x)
            ...
            if(flag_ordem):
                for j in range(0, self.cols):#coluna
                    ...
                    if(j+1<self.cols):
                        ...
                        while(controle_cnc.estado_atual(self.serial_cnc)!='Idle'):
                            time.sleep(0.125)
                        #time.sleep(self.tempo_entre_medidas) #para teste da tela atualizando
                flag_ordem=False
            else:
    ...
    def plot_dadoatual(self):
        ...
        if(self.flag_auto_maxmin):
            ...
            vmax=int(self.var_plot_max.get())    #função que verifica se é número
            vmin=int(self.var_plot_min.get())    #função que veririca se é número
